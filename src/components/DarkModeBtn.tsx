  ...
  return (
    <div>
      <input
        ...
        id="toggle"
        className="hidden"
        defaultChecked={(localStorage.getItem('isChecked') === 'true') ? true : false}
      ...
      <label htmlFor="toggle">
        <div className="flex h-8 w-16 items-center justify-between rounded-lg bg-gray-300 p-1.5">
          <div className="toggle-dot h-6 w-6 transform rounded-md bg-white duration-300 ease-in-out absolute z-10"></div>
          <BsSun />
          <BsMoonStars />
        </div>
      </label>
